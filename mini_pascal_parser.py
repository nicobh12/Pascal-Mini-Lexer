...
Complete Mini-Pascal parser using PLY (Python Lex-Yacc).
Layer 2 of the compiler pipeline: Lexer → Parser → Semantic Analyzer.

Grammar covers the full Mini-Pascal language subset:
  programs, blocks, declarations (label/const/type/var/subprogram),
  all statement forms, and expressions with correct Pascal precedence.
"""
import sys
...
from mini_pascal_lex import tokens, make_lexer   # noqa: F401 (tokens needed by PLY)
...
class ProcDecl:
    name: str
    params: List
    body: Any          # Block | None (if forward)
    forward: bool
    line: int = 0
...
class IfStmt:
    condition: Any
    then_branch: Any
    else_branch: Any   # None if no else
    line: int = 0
...
@dataclass
class RepeatStmt:
    body: List         # list of stmts (not CompoundStmt)
    condition: Any
    line: int = 0
...
# ============================================================
# Module-level parser state (reset on each parse() call)
# ============================================================
_parse_errors: List[ParseError] = []
_last_error_line: int = -1


def _record_parse_error(line: int, message: str) -> None:
    global _last_error_line
    if line == _last_error_line:
        return  # suppress duplicate-line spam
    _last_error_line = line
    _parse_errors.append(ParseError(kind='syntax_error', line=line, message=message))


# ============================================================
# Helper – build a Var/IndexVar/FieldVar/DerefVar from suffix list
# ============================================================
...
# assignment when lookahead after ID is ambiguous.

def p_id_stmt_assign(p):
    'id_stmt : id_prefix ASSIGN expression'
    target = p[1]
    # id_prefix always builds a Var/IndexVar/FieldVar/DerefVar node here
    p[0] = AssignStmt(target=target, value=p[3], line=p.lineno(2))
...
def p_id_stmt_call(p):
    ...
    if isinstance(node, FuncCall):
        ...
        # Complex lvalue used as statement — treat as no-op (error caught by semantic)
        p[0] = ProcCallStmt(name='?', args=[], line=0)
...
def p_id_prefix_var(p):
    ...
    p[0] = _build_var_node(p[1], p[2], line=p.lineno(1))
...
def p_relop(p):
    '''relop : EQUALS
             ...
             | GEQ
             | IN'''
    # Use the token TYPE name (e.g. 'GT', 'EQUALS') not the raw value ('>', '=')
    p[0] = p.slice[1].type
...
# Statement-level recovery: sync to ';' so one bad statement doesn't
# cascade.  Using SEMICOLON as sync-point means PLY discards tokens
# until it finds ';', then resumes — one error per statement.
def p_stmt_seq_error_semi(p):
    'stmt_seq : error SEMICOLON'
    p[0] = []


# repeat-stmt recovery: when UNTIL is missing, absorb remaining tokens up
# to whatever follows the repeat (END, ELSE, ';', '.').
def p_repeat_error(p):
    'repeat_stmt : REPEAT stmt_seq error'
    p[0] = RepeatStmt(body=p[2], condition=NilLit(), line=p.lineno(1))


_MAX_ERRORS = 20   # safety cap — stops recording after this many errors
...
def p_error(p):
    if p is None:
        ...
        return
    # Record the error (suppressed on duplicate lines or after cap)
    if len(_parse_errors) < _MAX_ERRORS:
        _record_parse_error(p.lineno, f"unexpected token {p.value!r}")
    # Do NOT call errok() here — doing so prevents PLY from finding the
    # error productions above, which handle synchronisation to ';'.
    # PLY will pop the stack and shift the 'error' token itself.
...
# Build the parser (tables not written to disk — no side-effects in tests)
_parser = yacc.yacc(
...
def parse(source: str) -> ParseResult:
    """Lex and parse *source*, returning a ParseResult with the AST."""
    global _parse_errors, _last_error_line
    _parse_errors = []
    _last_error_line = -1
    ...
    result.parse_errors = list(_parse_errors)
    return result
